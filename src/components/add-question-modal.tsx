...
export function AddQuestionModal({
  ...
  open: boolean
  onClose: () => void
  onAdd: (question: { text: string; type: QuestionType; options?: string[]; required?: boolean }) => void
}) {
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input placeholder="Enter question..." value={text} onChange={(e) => setText(e.target.value)} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-end gap-2">
              <Checkbox id="required" checked={required} onCheckedChange={(v) => setRequired(Boolean(v))} />
        ...
        <DialogFooter>
          ...
          <Button
            onClick={() => {
              if (!canSubmit) return
              onAdd({ text: text.trim(), type, options: type === "multiple-choice" ? options : undefined, required })
            }}
            disabled={!canSubmit}
          >
            Add Question
